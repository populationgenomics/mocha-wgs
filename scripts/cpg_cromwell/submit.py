...
OUTPUT_PREFIX = f'mocha-wgs/{JOB_NAME}'
DRIVER_IMAGE = _config['workflow']['driver_image']

b = get_batch()

input_prefix = 'MochaWgsPreprocess'

input_dict = {
    f'{input_prefix}.{k}': v
    for k, v in _config['mocha_wgs'].items()
}

submit_j, workflow_id_file = submit_cromwell_workflow(
    b=b,
    job_prefix='mocha-wgs-preprocess',
    dataset=DATASET,
    access_level=ACCESS_LEVEL,
    workflow='wgs_preprocess.wdl',
    output_prefix=OUTPUT_PREFIX,
    cwd='workflow/',
    input_dict=input_dict,
    libs=[
