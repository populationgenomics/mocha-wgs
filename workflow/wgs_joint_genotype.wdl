...
# This workflow is designed to run joint genotyping on one or more
# WGS gVCF files.

workflow WgsJointGenotyping {
    input {
        Array[File] gvcfs
    ...
    String merged_vcf_basename = basename(GenotypeGVCFs.gt_vcf[0], ".vcf.gz")

    call MergeVcfs as MergeGenotypedVcfs {
        input:
            vcfs = GenotypeGVCFs.gt_vcf,
            vcf_indexes = GenotypeGVCFs.gt_vcf_index,
    ...
    call ApplyVqsr {
        input:
            vcf = VariantFiltration.filtered_vcf,
            vcf_index = VariantFiltration.filtered_vcf_index,
            snp_recal = SnpRecalibrator.snp_recal,
            snp_re1cal_index = SnpRecalibrator.snp_recal_index,
            snp_tranches = SnpRecalibrator.snp_tranches,
            indel_recal = IndelRecalibrator.indel_recal,
            indel_recal_index = IndelRecalibrator.indel_recal_index,
            indel_tranches = IndelRecalibrator.indel_tranches,
            intervals = intervals,
            gatk_docker = gatk_docker,
            preemptible = preemptible,
            max_retries = max_retries,
    ...
    output {
        ...
        File filtered_vcf = ApplyVqsr.vqsr_vcf
        File filtered_vcf_index = ApplyVqsr.vqsr_vcf_index
    }
}
